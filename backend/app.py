...
app = FastAPI(title="Laxman AI Voice API", version="2.5.0")
...
def marker_chunks(text: str):
    """Return text/pause tokens so inline [pause:N] occurs at the exact position."""
...
def synth_text(text,voice,language,speed,pitch):
    cleaned=strip_emphasis(text).strip()
    if not cleaned:return None,None
    request=GenerateRequest(text=cleaned,voice=voice,language=language,speed=speed,pitch=pitch,normalize=False)
...
def synth_scene(text,voice,language,speed,pitch, trim=False, normalize=True, fade_in_ms=0, fade_out_ms=0, target_dbfs=-1.0):
    chunks=[]; sr=None
    for kind,value in marker_chunks(text):
        if kind=="pause":
            if sr is None: continue
            chunks.append(make_silence(sr,value)); continue
        data,chunk_sr=synth_text(value,voice,language,speed,pitch)
...
@app.get("/presets")
def presets():
    return {"presets":[{"id":"documentary","name":"Documentary","speed":0.95,"silence_ms":500},{"id":"news","name":"News","speed":1.08,"silence_ms":280},{"id":"youtube","name":"YouTube","speed":1.0,"silence_ms":350},{"id":"shorts","name":"Shorts / Reels","speed":1.18,"silence_ms":180},{"id":"education","name":"Education","speed":0.92,"silence_ms":450}]}
...
def generate(request:GenerateRequest):
    ...
    try:
        audio,sr=synth_scene(request.text,request.voice,request.language,request.speed,request.pitch,request.trim_silence,request.normalize,request.fade_in_ms,request.fade_out_ms,request.target_dbfs)
...
def scene_generate(request:SceneRequest):
    try: data,sr=synth_scene(request.text,request.voice,request.language,request.speed,request.pitch,request.trim_silence,request.normalize,request.fade_in_ms,request.fade_out_ms,request.target_dbfs)
    except (NotImplementedError,RuntimeError) as exc: raise HTTPException(503,str(exc)) from exc
    parts=[]
...
def batch_generate(request:BatchRequest):
    presets={"documentary":(.95,500),"news":(1.08,280),"youtube":(1.0,350),"shorts":(1.18,180),"education":(.92,450)}
    ...
    for i,scene in enumerate(raw_scenes,1):
        try:data,scene_sr=synth_scene(scene,request.voice,request.language,request.speed,1.0,request.trim_silence,request.normalize,request.fade_in_ms,request.fade_out_ms,request.target_dbfs)
        except (NotImplementedError,RuntimeError) as exc:raise HTTPException(503,str(exc)) from exc
        sr=sr or scene_sr
    ...
    manifest={"version":"2.5","preset":request.preset,"scenes":len(raw_scenes),"silence_ms":request.silence_ms,"language":request.language,"voice":request.voice,"speed":request.speed,"audio_processing":{"trim_silence":request.trim_silence,"normalize":request.normalize,"fade_in_ms":request.fade_in_ms,"fade_out_ms":request.fade_out_ms,"target_dbfs":request.target_dbfs},"total_duration_seconds":round(elapsed,3),"timeline":[{"id":i,"start":round(a,3),"end":round(b,3),"text":s} for i,(a,b,s) in enumerate(timeline,1)]}
    out=io.BytesIO()
