...
app = FastAPI(title="Laxman AI Voice API", version="2.2.0")
...
@app.get("/health")
def health():
    return {"ok": True, "version": "2.2.0", "engines": {"nepali_vits": {"ready": nepali_engine.adapter.ready, "loading": nepali_engine.adapter.loading, "error": nepali_engine.adapter.error}, "kokoro": kokoro_engine.adapter.ready}, "features": {"tts": True, "projects": True, "batch": True, "srt": True, "combined_wav": True, "openai_compatible": True, "voice_cloning": False}}
...
def batch_generate(request: BatchRequest):
    ...
    for i, scene in enumerate(scenes, 1):
        ...
        timeline.append((elapsed, elapsed+duration, scene))
        combined_parts.append(data)
        elapsed += duration
        if i < len(scenes) and request.silence_ms:
            silence_samples=int(sr*request.silence_ms/1000)
            combined_parts.append(np.zeros(silence_samples,dtype=np.float32))
            elapsed += request.silence_ms/1000
    ...
    with zipfile.ZipFile(out,"w",zipfile.ZIP_DEFLATED) as archive:
        ...
        archive.writestr("subtitles.srt",srt)
        archive.writestr("scenes.txt",scenes_txt)
        archive.writestr("manifest.json",__import__('json').dumps({"version":"2.2","scenes":len(scenes),"silence_ms":request.silence_ms,"language":request.language,"voice":request.voice},ensure_ascii=False,indent=2))
    out.seek(0)
...
def batch_status(): return {"supported": True, "mode": "server-side-scene-queue", "max_text_per_job": 100000, "max_scenes": 120, "outputs":["full-narration.wav","scene-wavs","subtitles.srt","scenes.txt","manifest.json"]}
@app.post("/clone")
